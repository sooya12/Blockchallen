...
- 기본 틀 div로 구성
- 나의 성공률 chart.js 사용해서 pie chart로 구현
- 지갑 생성을 위해서 web3로 생성까지 구현 (문제는 계정 생성이 eth0에서 안됨)
...
### :scroll: 20.09.15
 ...
 ##### 금창섭 멘토님과 블록체인 도메인 멘토링 진행

- 와이어프레임으로 프로젝트 주제 소개
- 프로젝트에서 블록체인 사용할 부분에 대해 자문 구함
  - 챌린지 베팅 금액 분배 - 스마트 컨트랙트
  - 사진 인증 - SHA256
- 블록체인 질문
  - 블록체인 데이터 생명주기
  - 금융권의 블록체인
...
- 소셜 로그인 백에서 처리해서 DB에 사용자 정보 넣으면, 프론트에서 nickname 받아서 수정해야 하는데 nickname 받는 페이지에 account id값을 받아오는게 잘 안됨
- Vue에서 백의 return 값 받는 게 생각보다 까다로움
- Vue에서 router간 데이터를 전송하는게 까다로움 - 해결 필요

##### 마이페이지 progress bar 추가
